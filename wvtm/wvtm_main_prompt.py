...
promt_slow_videos_dir = 'assets/videos_slow/'
...
class main_window:
    ...
    def __init__(self, master):
        ...
        self.master.bind("<space>", self.affirmative)
        self.master.bind("<d>", self.negative)
        self.master.bind("<a>", self.playVideo)
        self.master.bind("<s>", self.playVideoSlow)
        ...
        self.state_flag_t = 0 ##1-session start, 2-stop session, 3-start record, 4-stop record, 5-discard, 6-next
        self.index = 0
        self.out_file = None
        self.view_funcs = []
        ...
        frame18 = tk.Frame(frame1)
        frame18.pack(fill=tk.BOTH, side=tk.BOTTOM)
        ...
        self.video_view = tk.Label(frame3, bg=color_1)
        self.video_view.pack(fill=tk.Y, expand=True)
        ...
        #--------------frame17----------------
        self.play_vid_btn = tk.Button(frame17, text="Play Video", height=5, width=40, bg=color_white, fg=color_text_1, command=self.playVideo)
        self.play_vid_btn.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=10, pady=10)

        self.play_vid_slow_btn = tk.Button(frame17, text="Play Slow", height=5, width=40, bg=color_white, fg=color_text_1, command=self.playVideoSlow)
        self.play_vid_slow_btn.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=10, pady=10)

        #--------------frame18----------------
        self.stop_se = tk.Button(frame18, text="Stop Session", height=5, width=80, bg=color_white, fg=color_text_1, command=self.stopSession)
        ...
        def tick():
            ...
            if self.start_time_set:
                ...
                if (datetime.now() - self.start_time).total_seconds() >= 1.9:
    ...
    def startRecord(self):
        thread = threading.Thread(target=self.playVideo, args=(None, self.startRecProcess))
        thread.daemon = 1
        thread.start()
        self.state_buttons['start']["state"] = "disabled"
        self.state_buttons['start'].config(bg=color_white)
    ...
    def playCallback(self):
        self.startRecProcess()
    
    #play video
    def playVideo(self, action=None, callbackfn=None):
        try:
            player1 = tkvideo(os.path.join(promt_videos_dir, self.words_list[self.index].strip().replace('/', ' or ') + ".mp4"), self.video_playback_label, loop = 0, size = (650,900), hz=120, callback=callbackfn)
            player1.play()
            player1 = tkvideo(os.path.join(promt_videos_dir, self.words_list[self.index].strip().replace('/', ' or ') + ".mp4"), self.video_view, loop = 0, size = (325,450), hz=120)
            player1.play()
        except:
            pass
    
    def playVideoSlow(self, action=None, callbackfn=None):
        try:
            player1 = tkvideo(os.path.join(promt_slow_videos_dir, self.words_list[self.index].strip().replace('/', ' or ') + ".mp4"), self.video_playback_label, loop = 0, size = (650,900), hz=120, callback=callbackfn)
            player1.play()
            player1 = tkvideo(os.path.join(promt_slow_videos_dir, self.words_list[self.index].strip().replace('/', ' or ') + ".mp4"), self.video_view, loop = 0, size = (325,450), hz=120)
            player1.play()
        except:
            pass
    ...
    def affirmative(self, action):
        if self.state_flag_t in [1, 5, 6]:
            self.startRecord()
        elif self.state_flag_t == 3:
            self.stopRecord()
        elif self.state_flag_t == 4:
            self.next()
    
    def negative(self, action):
        if self.state_flag_t in [4, 5]:
            self.discard()
